...
export const CharactersTableRow = ({ character, champions, setChampions }: { character: Character, champions: Character[], setChampions: any }) => {
  const {updateChampions} = useChampions({characters: champions, setChampions})

  const isChampion = !!champions.find(champion => champion.name.toUpperCase() === character.name.toUpperCase());
  return (
    <TableRow key={character.name} sx={{ '&:last-child td, &:last-child th': { border: 0 } , backgroundColor: isChampion ? '#EDF5FF' : 'inherit'}}>
